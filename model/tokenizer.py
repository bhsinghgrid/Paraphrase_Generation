...
class SanskritTokenizer:
    ...
    # Inside your model/tokenizer.py
    # def decode(self, ids):
    #     # Filter out 0 (MASK), 1 (PAD), 2 (START), etc.
    #     clean_ids = [i for i in ids if i > 3]
    #     return self.tokenizer.decode(clean_ids)
    # model/tokenizer.py
    def decode(self, ids):
        # Ensure ids is a single list of integers
        if isinstance(ids[0], list):
            raise TypeError(
                "decode() expects a 1D list of IDs, but got a 2D list. Use a list comprehension in your trainer.")

        # Filter out special tokens: 0=MASK, 1=PAD, 2=START, 3=END
        clean_ids = [i for i in ids if i > 3]
        return self.tokenizer.decode(clean_ids)
