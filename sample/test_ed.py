...
#
CONFIG = {
    "model_type": "d3pm_encoder_decoder",  # change if needed
...
#     with open("results1/test_metrics.json", "w") as f:
...
def run_test(test_size=5000):
    ...
    model_path = f"production_model3/best_{CONFIG['model_type']}_neg.pt"
    if not os.path.exists(model_path):
        raise FileNotFoundError(f"Model not found: {model_path}")
    ...
    # Save JSON
    os.makedirs("../results1", exist_ok=True)
    with open("../results1/test_metrics_neg.json", "w") as f:
