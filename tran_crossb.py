...
#     # from bert_score import score
#     # HF_TOKENIZER = AutoTokenizer.from_pretrained("bert-base-multilingual-cased",use_fast=False)
...
#     # def compute_bertscore(self):
#     #     """
#     #     Compute BERTScore on validation set
#     #     Compatible with older bert_score versions
#     #     """
#     #
#     #     if not BERTSCORE_AVAILABLE:
#     #         return 0.0
...
#     #             preds = [
#     #                 self.tokenizer.decode(
#     #                     [id for id in ids.tolist() if id != CONFIG["diffusion"]["mask_token_id"]]
...
#     #         return 0.0
#     #
#     #     # 🔥 BERTScore call compatible with old versions
#     #     # Do NOT pass tokenizer=
#     #     P, R, F1 = score(
#     #         predictions,
#     #         references,
#     #         model_type="bert-base-multilingual-cased",
#     #         lang="hi",  # Hindi/Sanskrit compatible
#     #         device=str(self.device),
#     #         verbose=False
#     #     )
#     #
#     #     return F1.mean().item()
#
#     def compute_semantic_f1(self):
#         """
#         Computes semantic similarity F1 using sentence-transformers.
#         Works as a drop-in replacement for BERTScore.
#         """
#         self.model.eval()
#         predictions, references = [], []
#
...
#                 generated = self.model.model.generate(input_ids)
#
#                 # Decode sequences safely (without skip_special_tokens)
#                 preds = [
#                     self.tokenizer.decode([id for id in ids.tolist() if id != 0]).strip()
#                     for ids in generated
#                 ]
#
#                 predictions.extend(preds)
...
#         emb_preds = self.model.encode(predictions, convert_to_tensor=True)
#         emb_refs = self.model.encode(references, convert_to_tensor=True)
#         cos_sim_matrix = self.util.cos_sim(emb_preds, emb_refs)
#         semantic_scores = cos_sim_matrix.diag()
#         return semantic_scores.mean().item()
#
#     # -----------------------------------------------------
#     # MAIN TRAIN LOOP
#     # -----------------------------------------------------
...
#             bert_f1 = self.compute_semantic_f1()
#
...
class SanskritTrainer:
    ...
    # Initialize once globally (or inside Trainer init)
    SEMANTIC_MODEL = SentenceTransformer("distiluse-base-multilingual-cased-v2")  # multilingual

    def compute_semantic_f1(self):
        """
        Compute a BERTScore-like semantic similarity using sentence-transformers.
        Works for baseline_cross_attention and encoder-decoder models.
        Returns F1-like similarity score (0.0 - 1.0)
        """
        self.model.eval()
        predictions, references = [], []
        ...
        emb_preds = self.SEMANTIC_MODEL.encode(predictions, convert_to_tensor=True, show_progress_bar=False)
        emb_refs = self.SEMANTIC_MODEL.encode(references, convert_to_tensor=True, show_progress_bar=False)

        # Cosine similarity matrix
        sims = self.util.cos_sim(emb_preds, emb_refs)

        # Take diagonal (predicted i vs reference i) as score
        semantic_f1 = sims.diag().mean().item()
        return semantic_f1
    ...
    def train(self):
        ...
        import json
        with open("production_model3/training_metadata_crossb.json", "w") as f:
