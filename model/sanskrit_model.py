...
class SanskritModel(nn.Module):
    ...
    @torch.no_grad()
    def generate_cached(self, src, **kwargs):
        if not hasattr(self.model, "generate_cached"):
            return self.generate(src, **kwargs)
        sig = inspect.signature(self.model.generate_cached).parameters
        filtered = {k: v for k, v in kwargs.items() if k in sig}
        return self.model.generate_cached(src, **filtered)
