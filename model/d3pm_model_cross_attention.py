...
class MultiHeadAttention(nn.Module):
    def __init__(self, d_model, n_heads, dropout=0.1):
        ...
        self.compact_dim = d_model // 2
        self.q_proj   = nn.Linear(d_model, d_model)
        self.k_proj   = nn.Linear(d_model, d_model)
        self.v_proj   = nn.Linear(d_model, d_model)
        # Optional compact cross-attention branch (Task-1 optimized checkpoints).
        self.compact_q_proj = nn.Linear(d_model, self.compact_dim)
        self.compact_k_proj = nn.Linear(self.compact_dim, self.compact_dim)
        self.compact_v_proj = nn.Linear(self.compact_dim, self.compact_dim)
        self.compact_out_proj = nn.Linear(self.compact_dim, d_model)
    ...
    def forward(self, q, k, v, mask=None):
        B, Lq, _ = q.size()
        Lk = k.size(1)
        q_base = self.q_proj(q)
        k_base = self.k_proj(k)
        v_base = self.v_proj(v)

        Q = q_base.view(B, Lq, self.n_heads, self.head_dim).transpose(1, 2)
        K = k_base.view(B, Lk, self.n_heads, self.head_dim).transpose(1, 2)
        V = v_base.view(B, Lk, self.n_heads, self.head_dim).transpose(1, 2)
        ...
        # Compact branch is only enabled for checkpoints that contain trained compact weights.
        if self.use_compact:
            c = self.compact_dim
            h_comp = max(1, self.n_heads // 2)
            d_comp = c // h_comp
            q_small = self.compact_q_proj(q)                    # [B, Lq, c]
            k_small = self.compact_k_proj(self.compact_q_proj(k))  # [B, Lk, c]
            v_small = self.compact_v_proj(self.compact_q_proj(v))  # [B, Lk, c]
            ...
            if mask is not None:
                scores_c = scores_c.masked_fill(mask.unsqueeze(1).unsqueeze(2), float('-inf'))
...
class D3PMCrossAttention(nn.Module):
    def __init__(self, cfg):
        ...
        # Semantic residual bottleneck (Task 1). Kept near-identity by residual add.
    ...
    def forward(self, src, tgt, t, x0_hint=None, inference_mode=False):
        ...
        return self.forward_cached(
            memory, src_pad_mask, tgt, t, x0_hint=x0_hint, inference_mode=inference_mode
        )
